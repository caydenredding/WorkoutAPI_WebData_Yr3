...
@router.post(
    "/exercise-logs/{exercise_log_id}/sets",
...
def create_set(exercise_log_id: int, payload: schemas.SetCreate, db: Session = Depends(get_db)):
    ...
    s = models.SetLog(exercise_log_id=exercise_log_id, reps=payload.reps, weight=payload.weight)
    db.add(s)
    db.commit()
    db.refresh(s)
    return s
